import type { SymbolInfo } from "./symbol_info.js";

// Lookup table which factors to use for which number of error correction codewords.
const factorSets: number[] = [5, 7, 10, 11, 12, 14, 18, 20, 24, 28, 36, 42, 48, 56, 62, 68];
...
const factors: number[][] = [
  ...
  [
    52, 190, 88, 205, 109, 39, 176, 21, 155, 197, 251, 223, 155, 21, 5, 172, 254, 124, 12, 181, 184,
    96, 50, 193,
  ],
  [
    211, 231, 43, 97, 71, 96, 103, 174, 37, 151, 170, 53, 75, 34, 249, 121, 17, 138, 110, 213, 141,
    136, 120, 151, 233, 168, 93, 255,
  ],
  [
    245, 127, 242, 218, 130, 250, 162, 181, 102, 120, 84, 179, 220, 251, 80, 182, 229, 18, 2, 4, 68,
    33, 101, 137, 95, 119, 115, 44, 175, 184, 59, 25, 225, 98, 81, 112,
  ],
  [
    77, 193, 137, 31, 19, 38, 22, 153, 247, 105, 122, 2, 245, 133, 242, 8, 175, 95, 100, 9, 167,
    105, 214, 111, 57, 121, 21, 1, 253, 57, 54, 101, 248, 202, 69, 50, 150, 177, 226, 5, 9, 5,
  ],
  [
    245, 132, 172, 223, 96, 32, 117, 22, 238, 133, 238, 231, 205, 188, 237, 87, 191, 106, 16, 147,
    118, 23, 37, 90, 170, 205, 131, 88, 120, 100, 66, 138, 186, 240, 82, 44, 176, 87, 187, 147, 160,
    175, 69, 213, 92, 253, 225, 19,
  ],
  [
    175, 9, 223, 238, 12, 17, 220, 208, 100, 29, 175, 170, 230, 192, 215, 235, 150, 159, 36, 223,
  ...
  [
    242, 93, 169, 50, 144, 210, 39, 118, 202, 188, 201, 189, 143, 108, 196, 37, 185, 112, 134, 230,
  ...
  [
    220, 228, 173, 89, 251, 149, 159, 56, 89, 33, 147, 244, 154, 36, 73, 127, 213, 136, 248, 180,
...
const moduloValue = 0x12d;

const log = new Int32Array(256);
const alog = new Int32Array(255);
...
// errorCorrection_encodeECC200 creates the ECC200 error correction for an encoded message.
export function errorCorrection_encodeECC200(
  ...
  const total = symbolInfo.getDataCapacity() + symbolInfo.getErrorCodewords();
  const sb = new Uint8Array(total);
  sb.set(codewords, 0);
  const blockCount = symbolInfo.getInterleavedBlockCount();
  if (blockCount === 1) {
    const ecc = createECCBlock(codewords, symbolInfo.getErrorCodewords());
    sb.set(ecc, codewords.length);
  } else {
    ...
    for (let block = 0; block < blockCount; block++) {
      ...
      const ecc = createECCBlock(Uint8Array.from(temp), errorSizes[block]!);
      let pos = 0;
...
function createECCBlock(codewords: Uint8Array, numECWords: number): Uint8Array {
  let table = -1;
  ...
  const ecc = new Int32Array(numECWords);
  for (let i = 0; i < codewords.length; i++) {
    const m = ecc[numECWords - 1]! ^ codewords[i]!;
  ...
  const eccReversed = new Uint8Array(numECWords);
  for (let i = 0; i < numECWords; i++) {
    eccReversed[i] = ecc[numECWords - i - 1]! & 0xff;
  }
  return eccReversed;
}
